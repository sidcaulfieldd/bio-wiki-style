...
export default function Scratch() {
  ...
  useEffect(() => {
    ...
    // ---- Paper-behind-the-gif layer -----------------------------------
    // One jitter (background-position pan + a random horizontal/vertical
    // mirror + a very subtle brightness/contrast wobble) precomputed per
    // gif frame, so re-scrubbing back over the same frame always lands on
    // the same "misaligned scan" instead of re-randomizing every tick.
    // No rotation — the paper's ruled lines always stay horizontal.
    type PaperJitter = { x: number; y: number; flipX: number; flipY: number; brightness: number; contrast: number };
    let paperJitters: PaperJitter[] = [];
    let lastPaperFrame = -1;

    function buildPaperJitters(count: number) {
      const arr: PaperJitter[] = [];
      for (let i = 0; i < count; i++) {
        arr.push({
          ...
          contrast: 0.97 + Math.random() * 0.06
        });
      }
      paperJitters = arr;
      lastPaperFrame = -1;
    ...
    function applyPaperFrame(i: number) {
      if (i === lastPaperFrame || !paperJitters[i]) return;
      lastPaperFrame = i;
      const j = paperJitters[i];
      paper.style.backgroundPosition = `${j.x}% ${j.y}%`;
      paper.style.transform = `translate(-50%, -50%) rotate(90deg) scale(${j.flipX}, ${j.flipY})`;
      paper.style.filter = `brightness(${j.brightness.toFixed(3)}) contrast(${j.contrast.toFixed(3)})`;
    }
    // ---------------------------------------------------------------------
      ...
      st = ScrollTrigger.create({
        ...
        onUpdate: (self) => {
          ...
          if (!inVideoPhase) {
            ...
            drawCurrentFrame();
            applyPaperFrame(Math.min(CONFIG.frameCount - 1, Math.max(0, Math.round(state.frameIndex))));
    ...
    (window as any).__applyPath = function applyPath(path: "joel" | "becca") {
      ...
      frames.length = 0;
      state.frameIndex = 0;
      buildPaperJitters(CONFIG.frameCount);
    ...
    // Paper layer: static styles that never change per-frame live here;
    // only backgroundPosition/transform/filter are touched in applyPaperFrame.
    paper.style.backgroundImage = `url("${CONFIG.paperUrl}")`;
    paper.style.backgroundRepeat = "no-repeat";
    paper.style.backgroundSize = `${CONFIG.paperZoom * 100}% ${CONFIG.paperZoom * 100}%`;
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#ffffff" }}>
      ...
      >
        {/* Paper-behind-the-gif layer. Sits above the pin's white
            background but below everything else (video, canvas, mute
            overlay, loader). Sized/rotated so a single page fills the
            screen edge-to-edge in landscape, with lines held horizontal
            (no rotation jitter — only pan, mirror, and a subtle
            brightness/contrast wobble per frame). */}
        <div
          ref={paperRef}
          style={{
            position: "absolute",
